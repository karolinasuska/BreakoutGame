...
Ball::Ball(const Vec2D& pos, float radius): mBBox(pos - Vec2D(radius, radius), radius *2.0f - 0.5f, radius * 2.0f - 0.5f), mVelocity(Vec2D::Zero)
{
...
{
	mBBox.MoveBy(mVelocity * MilisecondsToSeconds(dt));
}
